...
/**
 * When we hit a box with a ray, we need the normal at the point
 * of intersection. This is just a very simple way of telling from the
 * point of intersection and the box definition what the normal
 * probably is based on the distance between the box sides and the
...
/**
 * Compared to the previous tutorial, this function changed significantly.
 * Now we do not just shoot the primary ray from the eye/camera into the scene
 * and return whether that ray intersected a box.
 * 
 * This time, when a ray hit a box surface, we evaluate a simple Lambertian BRDF,
 * attenuate any light potentially propagating along this ray path and compute a
 * next ray direction within the hemisphere of the box surface. This ray is then
 * followed again until it hits anything. In the case the ray escapes the scene through
 * the open ceiling/roof it is assumed that it hit a light source.
 *
...
vec3 trace(vec3 origin, vec3 dir) {
   ...
   * everytime we hit a box surface, we need to remember the attenuation the light
   * would take because of the albedo (fraction of light not absorbed) of the
   * surface it reflected off of. At the beginning, this value will be 1.0 since when
   * we start, the ray has not yet touched any surface.
   */
  vec3 att = vec3(1.0);
  /*
   * We are following a ray a fixed number of bounces through the scene.
   * Since every time the ray intersects a box, the steps that need to be done
   * at every such intersection are the same, we use a simple for loop.
   * The first iteration where `bounce == 0` will process our primary ray
   * starting directly from the eye/camera through the framebuffer pixel 
   * into the scene. Let's go...
   */
  for (int bounce = 0; bounce < 3; bounce++) {
    /*
     * The ray now travels through the scene of boxes and eventually either
     * hits a box or escapes through the open ceiling. So, test which case
     * it is going to be.
     */
    hitinfo hinfo;
    if (!intersectBoxes(origin, dir, hinfo)) {
       ...
       * the open ceiling to the outside world, which is assumed to consist
       * of nothing but light coming in from everywhere.
       * Return the attenuated sky color.
       */
     ...
     * equation `point = origin + t * dir` with 't' being the value returned
     * by intersectBoxes() in the hitinfo.
     */
    vec3 point = origin + hinfo.near * dir;
    /*
     * Now, we want to be able to generate a new ray which starts from the point
     * of intersection and travels away from the surface into a random direction.
     * For that, we first need the surface's normal at the point of intersection.
     */
    vec3 normal = normalForBox(point, b);
    /*
     * Next, we reset the ray's origin to the point of intersection offset by a 
     * small epsilon along the surface normal to avoid intersecting that box again
     * because of precision issues with floating-point arithmetic.
     * Notice that we just overwrite the input parameter's value since we do not
     * need the original value anymore, which for the first bounce was our eye/camera
     * location. 
     */
    origin = point + normal * EPSILON;
    /*
     * We have the new origin of the ray and now we just need its new direction.
     * Since we have the normal of the hemisphere within which we want to generate
     * the new direction, we can call a function which generates such vector based
     * on the normal and some pseudo-random numbers we need to generate as well.
     */
    dir = randomHemispherePoint(normal, randvec3(bounce));
    /*
     * The next thing we have to take care of is evaluating the surface BRDF with
     * the outgoing light ray direction (i.e. our previous 'dir') and the incoming
     * light direction (i.e. our newly computed 'dir'). Since we are dealing with
     * lambertian surfaces which reflect light equally in all directions, this BRDF
     * function is actually a constant equal to c/PI, where 'c' is the
     * albedo/reflectance factor of the surface determining how much of the incoming
     * light is reflected by the surface. We will take care of the 'c' factor
     * further below. Here we just multiply our attenuation factor by 1/PI.
     * In this tutorial the BRDF is a constant, however in later tutorials this will
     * be an actual function of the light direction vectors.
     */
    att *= ONE_OVER_PI;
    /*
     * Now this part is important: Any light that might come along the newly
     * generated ray has to be attenuated based on the angle between the generated
     * direction vector and the surface normal. This is known as the "cosine fall-off"
     * factor and is 100% physically correct and has actually nothing to do with
     * how the surface actually reflects light (diffuse/lambertian, specular, etc.).
     * The cosine term is also one part of the rendering equation.
     */
    att *= dot(dir, normal);
    /*
     * We also model the color of a box. And the color is nothing but the fraction
     * of incoming light which gets reflected off the surface and hence not absorbed
     * by the surface. This is often called the "albedo" of the surface.
     * To express this in our calculations, we further attenuate by the box's color.
     */
    att *= b.col;
    /*
     * The last factor we need to take care of is an important part of the Monte Carlo
     * integration. What we are doing is sampling the hemisphere of a surface with
     * random vectors and evaluate an integrand (in this case a lambertian BRDF).
     * If we take this integral for a given incident light direction, computing the
     * radiance for all outgoing directions, that integral must not be greater than 1.
     * This is to ensure energy conservation and means that a surface cannot reflect
     * more light than it receives. The integral should also be 1 for surfaces which
     * reflect all of its incoming light. For uniformly distributed hemisphere
     * directions (as in our case) the correction factor to achieve this is a
     * constant equal to 1/(2*PI).
     *
     * In the Monte Carlo estimator for an integrand f(x) and probability density
     * function p(x):
     *
     *  F[N] = 1/N * Sum{1, N}(f(x) / p(x))
     * 
     * this correction factor is precisely p(x) and we must therefore divide by it.
     *
     * In later tutorial parts when we do not use uniform hemisphere samples anymore
     * the probability density function is not going to be a constant but an actual
     * function depending on various input vectors.
     */
    att /= ONE_OVER_2PI;
  }
  /*
...
void main(void) {
   ...
   * accidentally write to or read from unallocated memory later.
   */
  ...
  /*
   * Trace the list of boxes with the initial ray `eye + t * dir` and follow it
   * along in the scene. The result is a computed color which we will combine with
   * the current/previous framebuffer content below.
   */
  vec3 color = trace(eye, normalize(dir));
  vec3 oldColor = vec3(0.0);
  /*
   * For any iteration but the very first, we load the current value of the pixel
   * from the framebuffer.
   */
  if (blendFactor > 0.0)
    oldColor = imageLoad(framebufferImage, px).rgb;
  /*
   * The final/new color to write to the framebuffer pixel is a linear interpolation
   * of the newly computed value in trace() and the old/current value based on
   * the blendFactor.
   */
  vec3 finalColor = mix(color, oldColor, blendFactor);
  /*
   * Store the final color in the framebuffer's pixel of the current work item.
   */
