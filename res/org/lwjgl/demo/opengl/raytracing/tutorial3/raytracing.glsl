...
/**
 * Compute the BRDF of the box's surface given the incoming and outgoing light
 * directions as well as the surface normal.
 *
...
/**
 * The general algorithm of this function did not change a lot compared to the 
 * last tutorial part. The only thing that changed is how we generate new
 * incoming light direction vectors. We use importance sampling here, which 
 * favors direction vectors for which the BRDF of the sampled surface has a 
 * large value. This will be used to significantly increase the rate of 
 * convergence of the image and thus reduce variance.
 *
...
vec3 trace(vec3 origin, vec3 dir) {
  ...
  for (int bounce = 0; bounce < 3; bounce++) {
    ...
    if (!intersectBoxes(origin, dir, hinfo)) {
       ...
       * hits the light source (which shall be a simple plane in our case).
       */
     ...
     * This time we will have two possible ways of generating a direction vector.
     * One is via a uniformly distributed hemisphere direction, like in the last
     * tutorial. Another way is by making use of our knowledge of the BRDF we are
     * using here, and generating vectors predominantly in those directions which
     * our BRDF would return the highest value (i.e. the lowest attenuation) of
     * the light.
    ...
    vec4 s;
    /*
     * Generate some random values we use later to generate the random vector.
     */
    ...
    if (importanceSampled) {
      /*
       * We will use importance sampling for the two parts, diffuse and specular,
       * of the BRDF separately. We do this via randomly choosing which
      ...
      if (rand.z < specularFactor) {
         ...
         * perfect reflection. Because Phong has a strong lobe around that
         * reflection vector we will generate most sample vectors around
        ...
        /*
         * When we are here, we should evaluate the diffuse part of the BRDF.
         * For that, we will use a cosine-weighted sample distribution.
         * This is because even though a lambertian/diffuse surface reflects
         * light in all directions that light is ultimately attenuated by
         * the cosine fall-off term of the rendering equation. So favoring 
         * the directions for which this cosine term is biggest is the best
         * we can do.
         */
        s = randomCosineWeightedHemispherePoint(normal, rand);
        /*
       ...
       * uniform hemisphere sampling. This will give a significantly worse
       * convergence rate with much more variance in the estimate/image.
       */
      s = randomHemispherePoint(normal, rand);
      /*
    ...
    /*
     * Use the newly generate direction vector as the next one to follow.
     */
    dir = s.xyz;
    /*
     * Attenuate by the cosine term.
     */
    att *= max(0.0, dot(dir, normal));
    /*
     * As mentioned above, our sample direction generation function now returns the
     * pdf(x) value for us in the w component of the return value. Also, make sure
     * that any value we get will not be 0.
     */
    if (s.w > 0.0)
      att /= s.w;
  }
